...
//
//     The number of email messages found in the string The date range of the
//     email messages
//
...
function getEmailDates(emails) {
  return emails.map((email) => email.split('#/#\n')[2])
}

function findDateRange(stringDates) {
  const dates = stringDates.map((date) => new Date(date.replace('Date: ', ''))).sort((a, b) => a - b)
  const minDate = dates[0].toDateString()
  const maxDate = dates[dates.length - 1].toDateString()
  return `Date Range: ${minDate} - ${maxDate}`
}

function mailCount(emailData) {
  const emails = emailData.split('##||##')
  console.log(`Count of Email: ${emails.length}`)
  console.log(findDateRange(getEmailDates(emails)))
}
