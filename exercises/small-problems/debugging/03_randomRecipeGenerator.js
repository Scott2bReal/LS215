...
console.log(`You need: ${dish.map(arr => arr.join(', ')).join(', ')}`);

// dishName is a string when we try to call join on it, so that is why it is
// not working. In order to make dishName an array (needed for join to work),
// we'll need to change how we initialize the variable. We need to create an
// array with multiple elements instead of just concatenating a string. We also
// need to change how we are displaying the dish ingredients, by joining each
// nested array into a string before joining the whole list.
