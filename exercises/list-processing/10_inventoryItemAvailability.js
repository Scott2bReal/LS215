...
function determineFinalAmount(transactions) {
  let sum = 0;

  transactions.forEach((transaction) => {
    transaction.movement === 'in' ? sum += transaction.quantity : sum -= transaction.quantity
  })
...
function isItemAvailable(id, transactions) {
  return determineFinalAmount(transactionsFor(id, transactions)) > 0;
}
